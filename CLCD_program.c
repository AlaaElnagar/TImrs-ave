...
void CLCD_VidSendString (char *Copy_pu8str)		/* Send string to LCD function */
{
	int i;
	for(i=0;Copy_pu8str[i]!=0;i++)		/* Send each char of string till the NULL */
	{
...
int CLCD_VidSendInt(s32 Copy_s64Integer)
{ static int a;
a=0;
if(Copy_s64Integer!=0)
{ CLCD_VidSendInt(Copy_s64Integer/10);
CLCD_VidSendData('0'+ Copy_s64Integer%10);
a++;
}
return a;
}
